...
/// Pure (hermetic, no-server) decoding tests for the model types. These use small
/// **hand-authored** JSON literals to exercise decoder edge cases that a vanilla
/// server run won't produce (multi-value headers/cookies, unknown delay types, the
/// `ids` snapshot output format, etc.).
///
/// For the *authoritative* real-server shapes — pinned against fixtures captured
/// from the pinned 3.13.2 server so they cannot silently drift — see
/// `RecordedContractTests` (fixtures under `Tests/WireMockTests/Fixtures/`,
/// regenerated by `Scripts/capture-fixtures.sh`). Live round-trips live in
/// `ModelDecodingLiveTests`.
