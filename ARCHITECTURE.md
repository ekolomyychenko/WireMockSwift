...
- **Клиент** (`WireMock`, DSL, верификация) поддерживается на macOS и iOS — только
  Foundation/`URLSession`; обе платформы гоняются в CI. Код кросс-платформенный (гварды
  `#if canImport(FoundationNetworking)` оставлены), но иные платформы не заявляются и не тестируются.
