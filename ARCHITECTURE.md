...
│  RequestPatternBuilder, Matchers (free-функции), HTTPMethod    │
└───────────────┬──────────────────────────────────────────────┘
...
└───────────────┬──────────────────────────────────────────────┘
                ...
                ▼
        ┌───────────────────────┐        ┌──────────────────────────┐
        │  Java-сервер WireMock  │◀──────▶│  WireMockServer (опц.)    │
        │  (jar / Docker)        │ spawn  │  запуск процесса из кода  │
        └───────────────────────┘        │  только macOS/Linux       │
                                          └──────────────────────────┘
```
...
### `WireMockServer` (`WireMockServer.swift`)
Опциональная обёртка для запуска сервера **из кода** (jar или Docker) через `Foundation.Process`.
Обёрнута в `#if os(macOS) || os(Linux)` — **на iOS отсутствует** (JVM/подпроцессы недоступны).
`start()` делает pre-flight проверку и отказывается стартовать, если порт уже занят чужим сервером
(иначе молча прицепился бы к чужому/устаревшему).
...
- `WireMockServer` — reference-тип, `@unchecked Sendable`; доступ к процессу под `NSLock`.
...
- **`WireMockServer`** — только macOS/Linux (запускает `java`/`docker` подпроцессом). На iOS его в
  сборке нет; там сервер поднимается **снаружи** (на хосте/CI), а клиент указывает на него по адресу.
...
- `WireMockServer(port:launch:)` — любой порт (`8080` лишь дефолт-удобство).
- Тесты и примеры читают адрес из `WIREMOCK_URL` (для XCUITest — через `.xctestplan`), что позволяет
  запускать несколько инстансов WireMock на разных портах (например, для параллельных сьютов).
...
Отдельно `Projects/WireMockXCUIDemo` — доказательство, что либа работает из XCUITest на симуляторе.
