...
> **Статус:** ранняя разработка (0.x). API ещё может меняться, лицензия пока не выбрана (см.
> [Лицензия](#лицензия)), и ни один релиз ещё не помечен тегом — поэтому пакет пока нельзя подключить как
> версионированную зависимость SwiftPM. Проверено на WireMock **3.13.2**.
